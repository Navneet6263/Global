...
import { ArrowUpRight, Layers, TriangleAlert, UsersRound } from "lucide-react";
import type { OpsMetric } from "../contracts/operations";
import { formatNumber } from "@/lib/formatting";

const ACTIONS = [
  { label: "Cases", icon: Layers, route: "/operations/cases" as const },
  { label: "Assign", icon: UsersRound, route: "/operations/assignments" as const },
  { label: "Exceptions", icon: TriangleAlert, route: "/operations/exceptions" as const },
];

interface OpsWorkloadCardProps {
  workload: OpsMetric;
  completed?: OpsMetric;
}

export function OpsWorkloadCard({ workload, completed }: OpsWorkloadCardProps) {
  return (
        ...
        <div className="space-y-3">
          ...
          {completed ? (
            <div className="flex items-baseline justify-between gap-2 rounded-[1.1rem] border border-white/80 bg-white/70 px-3 py-2.5 shadow-[var(--shadow-card)]">
              <span className="text-[11px] text-muted-foreground">{completed.label}</span>
              <span className="num text-base font-medium text-foreground">
                {formatNumber(completed.value)}
              </span>
            </div>
          ) : null}
        </div>
