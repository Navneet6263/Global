  ...
  try {
    ...
    const managedUser = await expectJson<{
      ...
      await api.post("users", {
        ...
        data: {
          ...
          roleCodes: ["VERIFIER"],
          temporaryPassword: "Managed!Password2026",
        },
      }),
      201,
    );
    ...
    const task = await expectJson<{ id: string; version: number }>(
      await api.post(`checks/${caseDetail.checks[0].publicId}/tasks`, {
        headers: writeHeaders(),
        data: { instructions: "Validate identity document and record the source." },
      }),
      201,
    );
    ...
    const decision = await expectJson<{
      ...
      await api.post(`qa/cases/${createdCase.id}/decision`, {
        ...
        data: {
          decision: "APPROVED",
          caseVersion: caseDetail.version,
          checklist: [
            "Consent evidence verified",
            "Identity source reviewed",
            "Result and report data checked",
          ],
    ...
    const generated = await expectJson<{
      ...
      await api.post(`cases/${createdCase.id}/reports/generate`, {
        headers: writeHeaders(),
      }),
      201,
    );
