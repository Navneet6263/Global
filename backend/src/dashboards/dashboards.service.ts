...
export class DashboardsService {
  ...
  async operations(actor: Actor) {
    ...
    const [byStatus, overdue, createdToday, completedToday, recentCases, outcomes, trend, atRisk] =
      await Promise.all([
    ...
    return {
      ...
      stageHealth: byStatus.map((row) => ({
        ...
        oldestAgeHours: row._min.updatedAt
          ? Math.max(0, Math.round((now.getTime() - row._min.updatedAt.getTime()) / 3_600_000))
          : 0,
        atRisk: atRisk.find((risk) => risk.status === row.status)?._count._all ?? 0,
      })),
      outcomeMix: Object.fromEntries(
        outcomes.map((row) => [row.result ?? "PENDING", row._count._all]),
      ),
      trend,
      recentCases: recentCases.map(({ publicId, ...row }) => ({ id: publicId, ...row })),
      generatedAt: now,
    };
  }
  ...
  private async trend(actor: Actor, now: Date) {
    return Promise.all(
      monthWindows(now).map(async ({ start, end, label }) => {
        const scope = caseAccessScope(actor);
