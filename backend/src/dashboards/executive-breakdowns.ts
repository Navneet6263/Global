...
export function stageAgeing(rows: ExecutiveCaseRow[], now: Date) {
  ...
  return [...groups.entries()]
    .map(([status, cases]) => {
      const ages = cases.map(
        (row) => (now.getTime() - row.updatedAt.getTime()) / 3_600_000,
      );
      return {
        status,
        count: cases.length,
...
export function teamCapacity(rows: ExecutiveCaseRow[]) {
  ...
  return [...groups.values()]
    .map((group) => ({
      id: group.id,
      name: group.name,
      active: group.rows.filter((row) => !terminalStatuses.includes(row.status))
        .length,
...
export function checkPerformance(rows: ExecutiveCaseRow[]) {
  ...
  return [...groups.entries()]
    .map(([type, checks]) => ({
      ...
      discrepancies: checks.filter((check) => check.result === "DISCREPANCY")
        .length,
      unableToVerify: checks.filter(
        (check) => check.result === "UNABLE_TO_VERIFY",
      ).length,
      averageTatHours: average(
        checks.flatMap((check) =>
          check.completedAt
            ? [
                (check.completedAt.getTime() - check.createdAt.getTime()) /
                  3_600_000,
              ]
            : [],
        ),
